...
for i in good bad diff; do
  ...
  $ROOT/tools/get-lines $working/$id/step-6/iter-$iter/$i/top.$k.index $working/$id/step-1/bad.clean.short.$input_lang > $working/$id/step-6/iter-$iter/$i/train.$input_lang
  $ROOT/tools/get-lines $working/$id/step-6/iter-$iter/$i/top.$k.index $working/$id/step-1/bad.clean.short.$output_lang > $working/$id/step-6/iter-$iter/$i/train.$output_lang
done
