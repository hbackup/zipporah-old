...
for lang in $input $output; do
  ...
  python $ROOT/scripts/collect-tags.py $working/$id/step-3/tagged/pos.list.$lang $corpus.$lang - | awk '{for(i=2;i<=NF;i+=2)printf("%s ",$i);print""}' > $working/$id/step-3/tagged/$file.$lang.pos.count
done
